...
///////////////////////////////////////////
  //  prob00 fvtx and prob05 sng - 2021 sim
  if(north_arm)
    ratio = 1199.54/13186.9;
  else
    ratio = 1061.93/8128.51;

  if(north_arm)
    fraction = 0.195/0.09064;
  else
    fraction = 0.231/0.10604;
///////////////////////////////////////////
...
void CBFitter_test_newbg_yuehang_psi2s_2nd_gaussian_check_sngdbl_mixed_YH_cent()
{

 // for 300 mev function descriptions of parameters a,b,d,e
 #include "tony_bestfit_parameters/yuehang_corrbg/yuehang_fit_coeff_300/fit_coeff_S_array_nineteen_point.C" 
 #include "tony_bestfit_parameters/yuehang_corrbg/yuehang_fit_coeff_300/fit_coeff_N_array_nineteen_point.C" 
	...
	{     
	  if(special == 0)
	      ...
	      else 
		{
		  ...
		  //////////////////////////////////////////////////////////////////////////////////////////////////////////////	
		   file1S = TFile::Open("psi2S_root_files/Run15pAu_sngdbl_psi2S_final_N.root");  // final result file
		   //////////////////////////////////////////////////////////////////////////////////////////////////////////////	
;
		}
	    } // special == 0
      ...
      else
	{
	  if(special == 0)
	      ...
	      else      
		{
		  ...
		  //////////////////////////////////////////////////////////////////////////////////////////////////////////////	
		   file1S = TFile::Open("psi2S_root_files/Run15pAu_sngdbl_psi2S_final_S.root");  // final result file
		   //////////////////////////////////////////////////////////////////////////////////////////////////////////////	
 ...
 double Npp = recomassBG2->IntegralAndError(recomassBG2->GetXaxis()->FindBin(2), recomassBG2->GetXaxis()->FindBin(5),dNpp,"");
  ...
  // double Npp = recomassBG2->IntegralAndError(recomassBG2->GetXaxis()->FindBin(2.5), recomassBG2->GetXaxis()->FindBin(5.5),dNpp,"");
  //  double Nmm = recomassBG1->IntegralAndError(recomassBG1->GetXaxis()->FindBin(2.5), recomassBG1->GetXaxis()->FindBin(5.5),dNmm,"");


  // double Npp = recomassBG2->IntegralAndError(recomassBG2->GetXaxis()->FindBin(1.5), recomassBG2->GetXaxis()->FindBin(4.5),dNpp,"");
  //  double Nmm = recomassBG1->IntegralAndError(recomassBG1->GetXaxis()->FindBin(1.5), recomassBG1->GetXaxis()->FindBin(4.5),dNmm,"");
 ...
 recomassBG->Scale(bgnorm);
  ...
  // for sys study
 /////////////////////////////////////////////////////////////////////////////////////////
 //  double Nmix = recomassBG3->IntegralAndError(recomassBG3->GetXaxis()->FindBin(2.5), recomassBG3->GetXaxis()->FindBin(5.5),dNmix,"");
 //  double mixnorm = 2*sqrt(Npp*Nmm)/Nmix;
 // recomassBG->Scale(bgnorm);
 //  recomassBG3->Scale(mixnorm);
   /////////////////////////////////////////////////////////////////////////////////////////
  // for sys study
 // /////////////////////////////////////////////////////////////////////////////////////////
 //  double Nmix = recomassBG3->IntegralAndError(recomassBG3->GetXaxis()->FindBin(1.5), recomassBG3->GetXaxis()->FindBin(5),dNmix,"");
 //  double mixnorm = 2*sqrt(Npp*Nmm)/Nmix;
 // recomassBG->Scale(bgnorm);
 //  recomassBG3->Scale(mixnorm);
   /////////////////////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////
  ...
  //bool write = true; 
   ...
   //////////////////////////////////////////////////////////////
      ...
      if(north_arm)
	{
	  ...
	  f7 = 1.15;  // as done in PPG188
	  //  f7 = 1.10;  // as done in PPG188
	  //	  f7 = 1.20;  // as done in PPG188
  ...
  if(fit == 1)
    {
     ...
     if(north_arm)
     {
       ...
       total_fit->SetParLimits(0, 0.5, 1.25);
       total_fit->SetParameter(1, f1);
        total_fit->SetParLimits(1, 0, 15);
     }
   else
     {
       total_fit->SetParameter(0, f0);
 ...
 if(fixed_width && fixed_mass)
    {
      if(north_arm)
	{
	  // MB, BG fit 1
	  total_fit->FixParameter(0, 1.00003);
	  total_fit->FixParameter(1, 15);
	  total_fit->FixParameter(2, 3.16768);  // Jpsi CB sigma
	  //  total_fit->FixParameter(2, 3.16768 + 2*0.00170853);  // Jpsi CB sigma
	  total_fit->FixParameter(3, 0.0975501);    // N J/psi
	...
	{
	  // BG fit 2
	  total_fit->FixParameter(0, 1.1659);
	  total_fit->FixParameter(1, 5);
	  total_fit->FixParameter(2, 3.12957);  // MB
	  // total_fit->FixParameter(2, 3.12957 + 2*0.00209418);    // for fixed psip mass
	  total_fit->FixParameter(3, 0.110754);   
...
TLegend *leg2 = new TLegend(0.65, 0.52, 0.84, 0.74);  //(start x, start y, end x, end y)
 ...
 if(bin == 1)
	...
	{
	  if(prob_cut)
	    sprintf(text3,"North p_{T} Int, MB");
	  else
	    sprintf(text3,"North p_{T} Int, MB");
	}
      if(north_arm == false)
	{
	  if(prob_cut)
	  sprintf(text3,"South p_{T} Int, MB");
	  else
	    sprintf(text3,"South p_{T} Int, MB");
	}
    }
...
if(bin == 2)
    {
      if(north_arm == true)
	{
	  sprintf(text3,"North p_{T} Int, 0-20%%");
	}
      if(north_arm == false)
	{
	  sprintf(text3,"South p_{T} Int, 0-20%%");
	}
    }
...
if(bin == 3)
    {
      if(north_arm == true)
	{
	  sprintf(text3,"North p_{T} Int, 20-40%%");
	}
      if(north_arm == false)
	{
	  sprintf(text3,"South p_{T} Int, 20-40%%");
	}
    }
...
if(bin == 4)
    {
      if(north_arm == true)
	{
	  sprintf(text3,"North p_{T} Int, 40-60%%");
	}
      if(north_arm == false)
	{
	  sprintf(text3,"South p_{T} Int, 40-60%%");
	}
    }
...
if(bin == 5)
    {
      if(north_arm == true)
	{
	  sprintf(text3,"North p_{T} Int, 60-84%%");
	}
      if(north_arm == false)
	{
	  sprintf(text3,"South p_{T} Int, 60-84%%");
	}
    }
...
if(bin == 6)
    {
      if(north_arm == true)
	{
	  sprintf(text3,"North p_{T} Int, 40-84%%");
	}
      if(north_arm == false)
	{
	  sprintf(text3,"South p_{T} Int, 40-84%%");
	}
    }
...
if(bin == 7)
    {
      if(north_arm == true)
	{
	  sprintf(text3,"North p_{T} Int, 20-84%%");
	}
      if(north_arm == false)
	{
	  sprintf(text3,"South p_{T} Int, 20-84%%");
	}
    }
 ...
 if(ppg188)
    {
      if(north_arm == true)
	//	sprintf(text3,"North p_{T} Int. (Local), SNGDBL");
...
l2.SetTextSize(0.035);
  ...
  sprintf(text7,"Case H");
  l3.SetTextAlign(12);
  l3.DrawLatexNDC(0.13, 0.69, text7);
