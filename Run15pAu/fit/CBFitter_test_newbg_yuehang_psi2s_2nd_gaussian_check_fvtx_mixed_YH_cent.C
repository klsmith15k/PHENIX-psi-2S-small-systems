...
Double_t CBcalc_jpsi(Double_t *xx, Double_t *par)  // CB fucntion for J/psi 
{
  ...
  // with FIDN 188 ONLY 
  if(north_arm)
    ratio = 692.158/8694.99;
  else
    ratio = 834.543/5703.68;
  if(north_arm)
    fraction = 0.191/0.08737;
  else
    fraction = 0.216/0.09815;
  /////////////////////////////////////////////
	...
	{     
	  if(special == 0)
	      ...
	      else 
		{
		  ...
		  //////////////////////////////////////////////////////////////////////////////////////////////////////////////	
		   file1S = TFile::Open("psi2S_root_files/Run15pAu_fvtx_psi2S_final_N.root");  // final result file
		   //////////////////////////////////////////////////////////////////////////////////////////////////////////////	
      ...
      else
	{
	  if(special == 0)
	      ...
	      else      
		{
		  ...
		  //////////////////////////////////////////////////////////////////////////////////////////////////////////////	
		   file1S = TFile::Open("psi2S_root_files/Run15pAu_fvtx_psi2S_final_S.root");  // final result file
		   //////////////////////////////////////////////////////////////////////////////////////////////////////////////	
 ...
 if(fixed_width && fixed_mass)
    {
      if(north_arm)
	{
	  total_fit->FixParameter(2,3.16960);
	  total_fit->FixParameter(3, 8.70364e-02);

	  // total_fit->FixParameter(2,3.17049);
	  // total_fit->FixParameter(3, 8.73845e-02);
	}
      else
	{
	  total_fit->FixParameter(2, 3.12936);  // Jpsi CB sigma
	  total_fit->FixParameter(3, 1.00678e-01);    // N J/psi

	  // total_fit->FixParameter(2, 3.13409);  // Jpsi CB sigma
	  // total_fit->FixParameter(3, 0.096356);    // N J/psi
	}
    }
...
pp_MB_S_total->GetXaxis()->SetRangeUser(2,5);
 ...
 // write histograms with fits to root file
  ...
  if(north_arm && trig)
    h = new TFile("YH_fit_hist_trig_N.root", "RECREATE");
  if(north_arm && !trig)
    h = new TFile("YH_fit_hist_notrig_N.root", "RECREATE");
  if(!north_arm && trig)
    h = new TFile("YH_fit_hist_trig_S.root", "RECREATE");
  if(!north_arm && !trig)
    h = new TFile("YH_fit_hist_notrig_S.root", "RECREATE");
