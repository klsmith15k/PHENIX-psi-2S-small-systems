...
void CB_macro_sanghoon_psi2s_2nd_G_sigmafree_toy()
{
  //////////////////////////////////////////////
  //  bool north_arm = true;
  bool north_arm = false;
  ...
  //   {
  //     if(sanghoon_files)
  // 	file1S = TFile::Open("sanghoon_simulations/mass_sngdbl_psi2s_dimu_02_noembedC_N.root");
  //     else
  //     file1S = TFile::Open("my_test_simulations/mass_sngdbl_psi2s_noembedC_N.root");
  ...
  //   {
  //     if(sanghoon_files)
  // 	file1S = TFile::Open("sanghoon_simulations/mass_sngdbl_psi2s_dimu_02_noembedC_S.root");
  //     else
  //     file1S = TFile::Open("my_test_simulations/mass_sngdbl_psi2s_noembedC_S.root");
...
ROOT::Math::MinimizerOptions::SetDefaultMaxFunctionCalls(100000); 
  ...
  total_fit->SetParameter(3, f3);
   ...
   total_fit->SetParameter(6, f6);
     
   sim_mass->Fit(total_fit,"","",2.0,4.4); 
   //sim_mass->Fit(total_fit,"L","",2,5); 
  ...
  for(Int_t i = 0;i < 6; i++)
    {
      for(Int_t j = 0;j < 6; j++)
	{
	  psi2s_array[6*i+j] = fullmat[6*i+j];
	  //cout << "psi2s array " << i << " " << j << " " << psi2s_array[i+j] << endl;
	}  
    }
 ...
 double X_mean_2nd;
  ...
  TF1 *psi2s_fit = new TF1("psi2s_fit",CBcalc_psi2s,2.0,4.4, 8);  
  ...
  Npsi2s = total_fit->Integral(2.0, 5.0)/width;  //  psi2s counts are calcualted from both the 2nd gaussian and the CB fit
 ...
 if(bin == 5)
    {
      if(north_arm == true)
	sprintf(text3,"p_{T} Int, Sng+Dbl Tracks");
      if(north_arm == false)
	sprintf(text3,"p_{T} Int, Sng+Dbl Tracks");
    };
...
double mean = total_fit->GetParameter(2);
 ...
 cout << "===========" << endl;
 cout <<  "N_2nd: " << total_fit->GetParameter(5) << endl;  
 cout << "N: " << total_fit->GetParameter(4) << endl;
  ...
  l3.SetTextAlign(12);
   ...
   l3.DrawLatexNDC(0.63, 0.79, text6); //4.4,150
   l3.DrawLatexNDC(0.63, 0.74, text8); //4.4,150
