...
// 4a for sng+dbl psi2s
 ...
 // // mixed events
  ...
  ///////////////////////////////////////////////////////////
    ...
    Double_t jpsi_fvtx_sys[narm][nbins] = {0};
    Double_t jpsi_sng_sys[narm][nbins] = {0};
    Double_t jpsi_sngdbl_sys[narm][nbins] = {0};
      
    for(int narm = 0; narm < 2; narm++)
      {
	for(int i = 0; i < 4; i++)
	  {
	    per_err = jpsi_ppg228_sys[narm][i] / RpAu_jpsi_ppg228_cent[narm][i];
	    if(arm == 0)
	      new_per_err = 2*per_err;
	    else
	      new_per_err = 1*per_err;
	    jpsi_fvtx_sys[narm][i] = new_per_err*RpAu_jpsi_fvtx_cent[narm][i];
	    jpsi_sng_sys[narm][i] = new_per_err*RpAu_jpsi_sng_cent[narm][i];
	    jpsi_sngdbl_sys[narm][i] = new_per_err*RpAu_sngdbl_jpsi_cent[narm][i];
	  }
      }
...
for(int i = 0; i < 4; i++)
     {
       ...
       gr5_sys->SetPoint(i, ncoll_array4[i],RpAu_sngdbl_psi2s_cent[1][i]);
       gr5_sys->SetPointError(i, width, width, psi2s_sys[1][i], psi2s_sys[1][i]);
   ...
   gr2_sys->SetFillStyle(0); 
    ...
    gr4_sys->SetMarkerColor(kBlack);                     
    gr4_sys->SetLineColor(kBlack);                         
 ...
 TCanvas *c1 = new TCanvas("c1","c1",0,0,800,545);
  ...
  gr1->GetXaxis()->SetNdivisions(5);
    ...
    l2.DrawLatexNDC(0.245, 0.90, text5); // x0, y0
