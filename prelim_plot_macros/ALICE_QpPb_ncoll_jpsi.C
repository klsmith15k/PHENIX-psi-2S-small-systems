...
void ALICE_QpPb_ncoll_jpsi()
{
  ...
  const Double_t ncoll_array1[6] = {8.1,6.0,4.3,2.5, 2, 13};  // need all 4 points for ppg228 array
  ...
  const Double_t ncoll_array6[6] = {12.7, 11.5, 9.81, 7.09, 4.28, 2.53};  // alice ncoll
  const Double_t ncoll_array7[6] = {9.7, 8.4, 7.4, 6.0, 4.3, 2.5};  // phenix ncoll jpsi
...
double pAu_MB[narm][nbins] = {
 ...
 // RUN15pp
	...
	jpsi_ppg228_pAu_inv_yield_3bins[arm][2] = (  (jpsi_ppg228_pAu_inv_yield[arm][2] * ncoll_array3[2]) + (jpsi_ppg228_pAu_inv_yield[arm][3] * ncoll_array3[3] ) ) / (  ncoll_array3[2] + ncoll_array3[3] );
	jpsi_ppg228_pAu_inv_yield_3bins_err[arm][2] = (  (jpsi_ppg228_pAu_inv_yield_err[arm][2] * ncoll_array3[2]) + (jpsi_ppg228_pAu_inv_yield_err[arm][3] * ncoll_array3[3] ) ) / (  ncoll_array3[2] + ncoll_array3[3] );
    ...
    double RpAu_jpsi_ppg228_cent[narm][6] = {
      {0.87, 0.86, 0.93, 0.88, 0.87, 0.81},
      {0.49, 0.60, 0.66, 0.76, 0.85, 0.91}};

    double RpAu_jpsi_ppg228_cent_err[narm][6] = {
 ...
 double sngdbl_jpsi[narm][nbins] = {
      ...
      
  {
	 ...
	 sngdbl_jpsi_acc_eff[0][cent] = e1->GetEfficiency(cent+1);
       ...
       sngdbl_jpsi_acc_eff[0][0] = 0.0247;
       sngdbl_jpsi_acc_eff[0][1] = 0.0259;
       sngdbl_jpsi_acc_eff[0][2] = 0.0268;
       sngdbl_jpsi_acc_eff[0][3] = 0.0294;
	 ...
	 sngdbl_jpsi_acc_eff[1][cent] = e2->GetEfficiency(cent+1);
       ...
       sngdbl_jpsi_acc_eff[1][0] = 0.0333;
       sngdbl_jpsi_acc_eff[1][1] = 0.0339;
       sngdbl_jpsi_acc_eff[1][2] = 0.0342;
       sngdbl_jpsi_acc_eff[1][3] = 0.0355;
    ...
    // RUN15pAu
     {
       ...
       sngdbl_jpsi_trig_eff[0][0] = 0.7794;
       sngdbl_jpsi_trig_eff[0][1] = 0.7137;
       sngdbl_jpsi_trig_eff[0][2] = 0.6930;
       sngdbl_jpsi_trig_eff[0][3] = 0.6716;
       ...
       sngdbl_jpsi_trig_eff[1][0] = 0.7479;
       sngdbl_jpsi_trig_eff[1][1] = 0.7533;
       sngdbl_jpsi_trig_eff[1][2] = 0.7531;
       sngdbl_jpsi_trig_eff[1][3] = 0.7464;
	...
	for(int bin = 0; bin < 4; bin++)
	  {
	    sngdbl_jpsi_pAu_inv_yield[arm][bin] = bias_228[bin] * sngdbl_jpsi[arm][bin] / ( ( FACTOR + sngdbl_jpsi_trig_eff[arm][bin] ) * ( FACTOR + sngdbl_jpsi_acc_eff[arm][bin] ) * pAu_MB[arm][bin] );
	    sngdbl_jpsi_pAu_inv_yield_err[arm][bin] = bias_228[bin] * sngdbl_jpsi_err[arm][bin] / ( ( FACTOR + sngdbl_jpsi_trig_eff[arm][bin]) * ( FACTOR + sngdbl_jpsi_acc_eff[arm][bin]) * pAu_MB[arm][bin] );
    ...
    double pp_sngdbl_jpsi[narm] = {18692,13235}; // mixed
	...
	for(int bin = 0; bin < 4; bin++)
	  {
	    ...
	    RpAu_sngdbl_jpsi_cent[arm][bin] =( sngdbl_jpsi_pAu_inv_yield[arm][bin] ) / ( ncoll_228[bin] * pp_sngdbl_jpsi_inv_yield[arm] ) ;
	    ...
	    cout << "RpAu sngdbl J/psi: " <<   RpAu_sngdbl_jpsi_cent[arm][bin]  << endl;
	  }
      }
...
// 4a for sng+dbl psi2s
 ...
 // // mixed events
    double sngdbl_psi2s[narm][nbins] = {
      {58,89,66,0},
      // {101,81,146,0}};  // three points North
      {94,81,146,0}};  // three points North
    ...
    
  {
       ...
       sngdbl_psi2s_acc_eff[0][0] = e1->GetEfficiency(1);
       sngdbl_psi2s_acc_eff[0][1] = e1->GetEfficiency(2);
       sngdbl_psi2s_acc_eff[0][2] = 0.0347;
       ...
       sngdbl_psi2s_acc_eff[1][0] = e2->GetEfficiency(1);
       sngdbl_psi2s_acc_eff[1][1] = e2->GetEfficiency(2);
       sngdbl_psi2s_acc_eff[1][2] = 0.0455;
    ...
    // RUN15pAu
     {
       ...
       sngdbl_psi2s_trig_eff[0][0] = f1->Eval(10);
       sngdbl_psi2s_trig_eff[0][1] = f1->Eval(30);
       sngdbl_psi2s_trig_eff[0][2] = 0.5994;
       ...
       sngdbl_psi2s_trig_eff[1][0] = f2->Eval(10);
       sngdbl_psi2s_trig_eff[1][1] = f2->Eval(30);
       sngdbl_psi2s_trig_eff[1][2] = 0.7673;
    ...
    for(int arm = 0; arm < 2; arm++)
 	...
 	for(int bin = 0; bin < 4; bin++)
 	  {
 	    sngdbl_psi2s_pAu_inv_yield[arm][bin] = bias[bin] * sngdbl_psi2s[arm][bin] / ( ( FACTOR + sngdbl_psi2s_trig_eff[arm][bin] ) * ( FACTOR + sngdbl_psi2s_acc_eff[arm][bin] ) * pAu_MB[arm][bin] );
    ...
    double pp_sngdbl_psi2s_inv_yield_err[narm] = {0}; 
     ...
     double pp_sngdbl_psi2s[narm] = {581,460};
    double pp_sngdbl_psi2s_err[narm] = {37,35};
    ...
    //RUN15pp
     {
       ...
       pp_sngdbl_psi2s_acc_eff[0] = 0.0416;
     ...
     // RUN15pp
     {
       ...
       pp_sngdbl_psi2s_trig_eff[0] = 0.7611;
       ...
       pp_sngdbl_psi2s_trig_eff[1] = 0.7346;
  ...
  ///////////////////////////////////////////////////////////
    // double ratio calcualtion
    //   psi(2S) RpAu / ppg228 RpAu

    // double dbl_ratio[2][4] = {0};
    // double dbl_ratio_err[2][4] = {0};
    // double dbl_ratio_sys[2][4] = {0};

    double ratio_pAu[2][4] = {0};
    double ratio_pAu_err[2][4] = {0};

    // double ratio_pp = 2.52;
    // double ratio_pp_err = 0.12;
	...
	for(int cent = 0; cent < 4; cent++)  // last bin is zero
	  {
	    ...
	    //dbl_ratio[arm][cent] = ratio_pAu[arm][cent] / ratio_pp;
	    ...
	    // tempRN = ratio_pAu_err[arm][cent] / ratio_pAu[arm][cent];
	    // tempRD = ratio_pp_err / ratio_pp;
	    // dbl_ratio_err[arm][cent] = sqrt( (tempRN * tempRN) + (tempRD * tempRD) ) * dbl_ratio[arm][cent];
    ...
    double ratio_alice[2][6] = {
      {1.156, 1.121, 1.057, 0.932, 0.851, 0.803},
      {0.692, 0.693, 0.720, 0.714, 0.759, 0.845}};

    double ratio_alice_err[2][6] = {
      {0.014, 0.011, 0.008, 0.008, 0.009, 0.016},
      {0.009, 0.009, 0.007, 0.008, 0.009, 0.018}};

    double ratio_alice_sys[2][6] = {
      {0.069, 0.054, 0.044, 0.051, 0.051, 0.033},
      {0.044, 0.035, 0.032, 0.041, 0.047, 0.040}};
    ///////////////////////////////////////////////////////////////////////////////


    const Double_t err_x[4] = {0,0,0, 0};
    const Double_t err_x6[6] = {0,0,0,0,0,0};
    const Double_t width = 0.3;
   

    const Double_t jpsi_ppg228_sys[2][6] = {  // 0-5, 5-10, 10-20
      {0.09, 0.09, 0.09, 0.09, 0.09, 0.08},
      {0.04, 0.05, 0.06, 0.07, 0.07, 0.08}};



    TGraphErrors *gr1 = new TGraphErrors(6,ncoll_array1,ratio_alice[1],err_x6, ratio_alice_err[1]);

    TGraphErrors *gr3 = new TGraphErrors(6,ncoll_array6,ratio_alice[1],err_x6, ratio_alice_err[1]);

    TGraphErrors *gr5 = new TGraphErrors(6,ncoll_array7, RpAu_jpsi_ppg228_cent[1],err_x6,  RpAu_jpsi_ppg228_cent_err[1] );


    TGraphErrors *gr7 = new TGraphErrors(4,ncoll_array4,RpAu_jpsi_sng_cent[1],err_x,RpAu_jpsi_sng_cent_err[1]);
    TGraphErrors *gr9 = new TGraphErrors(6,ncoll_array1,RpAu_jpsi_ppg228_cent[1],err_x, RpAu_jpsi_ppg228_cent_err[1]);
    ...
    TGraphErrors *gr6 = new TGraphErrors(6,ncoll_array7, RpAu_jpsi_ppg228_cent[0],err_x6, RpAu_jpsi_ppg228_cent_err[0]);


    TGraphErrors *gr8 = new TGraphErrors(4,ncoll_array4,RpAu_jpsi_sng_cent[0],err_x,RpAu_jpsi_sng_cent_err[0]);
    TGraphErrors *gr10 = new TGraphErrors(6,ncoll_array4,RpAu_jpsi_ppg228_cent[0],err_x, RpAu_jpsi_ppg228_cent_err[0]);
	...
	gr5_sys->SetPoint(i, ncoll_array7[i],RpAu_jpsi_ppg228_cent[1][i]);
	gr5_sys->SetPointError(i, width, width, jpsi_ppg228_sys[1][i], jpsi_ppg228_sys[1][i]);

	gr6_sys->SetPoint(i, ncoll_array7[i],RpAu_jpsi_ppg228_cent[0][i]);
    ...
    gr1->GetXaxis()->SetRangeUser(0.1,13.9);
    ...
    gr1->GetYaxis()->SetTitle("R_{pAu},Q_{pPb}");
    ...
    TLine *line = new TLine(0.9,1,13.85, 1);
    line->SetLineWidth(4);
    ...
    double global_sys = 0.101;  

    TBox *box = new TBox(13.75, 1-global_sys, 13.95, 1+global_sys);
 
    box->SetFillStyle(1000);
    box->SetFillColorAlpha(kRed+1,0.5);
    box->Draw();

    double global_sys0 = 0.084;  

    TBox *box0 = new TBox(13.55, 1-global_sys0, 13.75, 1+global_sys0);
    box0->SetFillStyle(1000);
    box0->SetFillColorAlpha(13,0.5);
    box0->Draw();


    double global_sys2 = 0.020;  

    TBox *box2 = new TBox(13.35, 1-global_sys2, 13.55, 1+global_sys2);
    box2->SetFillStyle(1000);
    box2->SetFillColorAlpha(1,0.5);
    box2->Draw();


    TLegend *leg = new TLegend(0.18, 0.83.5, 0.44, 0.86);  //(start x, start y, end x, end y)
    ...
    if(north_arm)
      c1->SaveAs(Form("../psi2S_pdf/RpAu_jpsi_alice_plot_fwd.pdf"));
    else
      c1->SaveAs(Form("../psi2S_pdf/RpAu_jpsi_alice_plot_bkwd.pdf"));
