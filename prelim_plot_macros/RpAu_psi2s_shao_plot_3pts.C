...
void RpAu_psi2s_shao_plot_3pts()
{
    gROOT->ForceStyle();
  ...
  double cent_binwidth[4] = {0.20, 0.20, 0.44, 0};
...
double pAu_MB[narm][nbins] = {
 ...
 // 4a for sng+dbl Jpsi
      ...
      
  {
	...
	for(int bin = 0; bin < 4; bin++)
	  {
	    FACTOR = 0;  // prob cut on both arms and both tracks
...
////////////////////////////// calcualte weighted MB sum
  ...
  //====================================================================================
    ...
    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    {
       file_data1 = TFile::Open("psi2S_root_files/psi2s_RpAu_ncoll_shao_N.root");
       file_data1->GetObject("RpAu_ncteq_shao_ncoll_fwd",p2);
       file_data1->GetObject("RpAu_epps_shao_ncoll_fwd",p1);
   ...
   for(int i = 0; i < 4; i++)
     {
       ...
       gr4_sys->SetPoint(i, ncoll_array4[i],RpAu_sngdbl_psi2s_cent[0][i]);
       gr4_sys->SetPointError(i, width, width, psi2s_sys[0][i], psi2s_sys[0][i]);
    ...
    gr3_sys->SetMarkerColor(kBlue+1);                     
    gr3_sys->SetLineColor(kBlue+1);                         
    gr3_sys->SetFillStyle(0); 
    gr3_sys->SetLineWidth(1);

    gr4_sys->SetMarkerColor(kBlue+1);                     
    gr4_sys->SetLineColor(kBlue+1);                         
    ...
    gr5->SetMarkerColorAlpha(kBlack, 0.8);                     
    gr5->SetLineColorAlpha(kBlack, 0.8);                         
    gr5->SetLineWidth(3);                           
    gr5->SetMarkerStyle(20);                       
    gr5->SetMarkerSize(2); 

    gr9->SetMarkerColorAlpha(kBlue+1, 0.8);
    gr9->SetLineWidth(3);
    gr9->SetLineColorAlpha(kBlue+1, 0.8);
    ...
    /////////////////// predicitons - psi(2S) North
 
    p1->SetFillStyle(3001);
    p1->SetLineColor(kBlue+1);
    p1->SetFillColorAlpha(kBlue+1, 0.3);
    p1->SetLineWidth(1);

    /////////////////// predicitons - J/psi North
	...
	leg->AddEntry(gr5,"R_{pAu} J/#psi, p+Au #sqrt{s_{NN}}=200 GeV", "p");
	...
	//	leg2->AddEntry(gr6,"R_{pAu} J/#psi PPG228 No Fvtx", "p");
	//	leg2->AddEntry(gr8,"R_{pAu} J/#psi Sng Tracks", "p");
	leg2->AddEntry(gr10,"R_{pAu} #psi(2S) Sng+Dbl Tracks", "p");
	//	leg2->AddEntry(p2,"J/#psi Transport Model (Du&Rapp)", "f");
	leg2->AddEntry(p6,"#psi(2S) nCTEQ15 (Shao et al)", "f");
 ...
 if(north_arm)
      c1->SaveAs(Form("../psi2S_pdf/RpAu_psi2s_shao_plot_fwd.pdf"));
    else
      c1->SaveAs(Form("../psi2S_pdf/RpAu_psi2s_shao_plot_bkwd.pdf"));


 // write TGraphs to same root file

  TFile *h;

  h = new TFile("RpAu_psi2s_shao_ncoll_N.root", "RECREATE");
