...
void RpAu_psi2s_transport_plot_3pts()
{
    gROOT->ForceStyle();
...
// 4a for sng+dbl psi2s
 ...
 // // mixed events
  ...
  //====================================================================================
   ...
   for(int i = 0; i < 4; i++)
    ...
    gr4_sys->SetMarkerColor(kRed+1);                     
    gr4_sys->SetLineColor(kRed+1);                         
    ...
    gr9->SetMarkerColorAlpha(kRed+1, 0.8);
    gr9->SetLineWidth(3);
    gr9->SetLineColorAlpha(kRed+1, 0.8);
    ...
    p3->SetFillStyle(3001);
      p3->SetLineColor(kRed+1);
    p3->SetFillColorAlpha(kRed+1, 0.3);
    p3->SetLineWidth(1);
	...
	leg2->AddEntry(p6,"J/#psi Transport Model(Du&Rapp)", "f");
 ...
 // write TGraphs to same root file
  ...
  h = new TFile("RpAu_psi2s_transport_ncoll_N.root", "RECREATE");
