...
void ALICE_QpPb_ncoll_2bins()
{
  ...
  const Double_t ncoll_array1[nbins] = {1.9,6.0,4.3,13.0};  // need all 4 points for ppg228 array
  const Double_t ncoll_array2[nbins] = {8.3,6.2,4.5,2.7};
  const Double_t ncoll_array3[nbins] = {8.2,6.1,4.4,2.6};
  const Double_t ncoll_array4[nbins] = {8.2,4.3, 0,0};
...
double pAu_MB[narm][nbins] = {
 ...
 TFile *file_data1, *file_data2;
  ...
  TEfficiency *e2;
	...
	jpsi_ppg228_pAu_inv_yield_3bins[arm][2] = (  (jpsi_ppg228_pAu_inv_yield[arm][1] * ncoll_array3[1]) + (jpsi_ppg228_pAu_inv_yield[arm][2] * ncoll_array3[2]) + (jpsi_ppg228_pAu_inv_yield[arm][3] * ncoll_array3[3] ) ) / (   ncoll_array3[1] + ncoll_array3[2] + ncoll_array3[3] );
	jpsi_ppg228_pAu_inv_yield_3bins_err[arm][2] = (  (jpsi_ppg228_pAu_inv_yield_err[arm][1] * ncoll_array3[1]) + (jpsi_ppg228_pAu_inv_yield_err[arm][2] * ncoll_array3[2]) + (jpsi_ppg228_pAu_inv_yield_err[arm][3] * ncoll_array3[3] ) ) / (   ncoll_array3[1] + ncoll_array3[2] + ncoll_array3[3] );
 ...
 // // mixed events
    ...
    
  {
    ...
    // RUN15pAu
     {
       ...
       sngdbl_psi2s_trig_eff[0][0] = 0.7840;  // updated bkgd hits
       sngdbl_psi2s_trig_eff[0][1] = 0.6477; // updated bkgd hits
	 sngdbl_psi2s_trig_eff[0][2] = 0;
       ...
       sngdbl_psi2s_trig_eff[1][0] = 0.7642; // updated bkgd hits
       sngdbl_psi2s_trig_eff[1][1] = 0.7735; // updated bkgd hits
       sngdbl_psi2s_trig_eff[1][2] = 0.7718; // updated bkgd hits
    ...
    //RUN15pp
     ...
     // RUN15pp
     {
       ...
       pp_sngdbl_psi2s_trig_eff[0] = 0.7637; // updated bkgd hits
       ...
       pp_sngdbl_psi2s_trig_eff[1] = 0.7350; // updated bkgd hits
  ...
  ///////////////////////////////////////////////////////////
    ...
    double ratio_alice[2][6] = {
      {1.017, 0.580, 0.742, 0.516, 0.450, 0.652},
      {0.858, 0.514, 0.471, 0.619, 0.697, 1.065}};

    double ratio_alice_err[2][6] = {
      {0.197, 0.173, 0.123, 0.126, 0.131, 0.226},
      {0.159, 0.144, 0.107, 0.119, 0.146, 0.271}};

    double ratio_alice_sys[2][6] = {
      {0.092, 0.053, 0.077, 0.050, 0.055, 0.064},
      {0.096, 0.053, 0.044, 0.060, 0.082, 0.095}};
    ///////////////////////////////////////////////////////////////////////////////
...
const Double_t psi2s_percent[narm][nbins] = {
    ...
    double per_err = 0;
    double new_per_err = 0;

   

    TGraphErrors *gr1 = new TGraphErrors(4,ncoll_array1,jpsi_ppg228[0], err_x, jpsi_ppg228_err[0]);
    TGraphErrors *gr3 = new TGraphErrors(6,ncoll_array6,ratio_alice[1],err_x6, ratio_alice_err[1]);
    TGraphErrors *gr5 = new TGraphErrors(3,ncoll_array4,RpAu_sngdbl_psi2s_cent[1],err_x, RpAu_sngdbl_psi2s_cent_err[1] );

    TGraphAsymmErrors *gr1_sys = new TGraphAsymmErrors(4);
    TGraphAsymmErrors *gr3_sys = new TGraphAsymmErrors(6);
    TGraphAsymmErrors *gr5_sys = new TGraphAsymmErrors(4);  // psi(2S) sys err for PHENIX
    TGraphAsymmErrors *gr7_sys = new TGraphAsymmErrors(4);
  
   
    TGraphErrors *gr2 = new TGraphErrors(4,ncoll_array1,jpsi_ppg228[0], err_x, jpsi_ppg228_err[0]);
    TGraphErrors *gr4 = new TGraphErrors(6,ncoll_array6,ratio_alice[0],err_x6, ratio_alice_err[0]);
    TGraphErrors *gr6 = new TGraphErrors(3,ncoll_array4, RpAu_sngdbl_psi2s_cent[0],err_x, RpAu_sngdbl_psi2s_cent_err[0]);
...
for(int i = 0; i < 4; i++)
     {
       ...
       gr6_sys->SetPoint(i, ncoll_array4[i],RpAu_sngdbl_psi2s_cent[0][i]);
       gr6_sys->SetPointError(i, width, width, psi2s_sys[0][i], psi2s_sys[0][i]);

     }

   gr1_sys->SetMarkerColor(kBlue+1);                     
   gr1_sys->SetLineColor(kBlue+1);                         
   gr1_sys->SetFillStyle(0); 
   
   gr2_sys->SetMarkerColor(kBlue+1);                     
   gr2_sys->SetLineColor(kBlue+1);                         
 ...
 TCanvas *c1 = new TCanvas("c1","c1",0,0,800,545);
  ...
  gr1->GetXaxis()->SetNdivisions(5);
	...
	sprintf(text5,"Inclusive #psi(2S)");
    ...
    l2.SetTextFont(102);  
     l2.DrawLatexNDC(0.245, 0.635, text3); // x0, y0
    l2.DrawLatexNDC(0.24, 0.90, text5); // x0, y0
    l2.DrawLatexNDC(0.24, 0.796, text4); // x0, y0
    l2.SetTextSize(0.04);
 ...
 if(north_arm)
      c1->SaveAs(Form("../psi2S_pdf/RpAu_psi2s_alice_plot_fwd.pdf"));
    else
      c1->SaveAs(Form("../psi2S_pdf/RpAu_psi2s_alice_plot_bkwd.pdf"));
