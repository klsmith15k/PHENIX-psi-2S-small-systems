...
void yuehang_minbias_fx_macro_xuan_ratio()
{
  bool pt_direct_int = true;
  bool pt_mass_ratio = false; 
...
//for each refit evaluated at each mass bin
...
#include "pt_int/fit_array_1gev_S.C" 
  ...
  // for Yue Hang direct pt int sum
#include "pt_int/yuehang_direct_pt_int_200mev_N.C"
#include "pt_int/yuehang_direct_pt_int_200mev_S.C"
#include "pt_int/yuehang_direct_pt_int_300mev_N.C"
#include "pt_int/yuehang_direct_pt_int_300mev_S.C"
#include "pt_int/yuehang_direct_pt_int_1gev_N.C"
#include "pt_int/yuehang_direct_pt_int_1gev_S.C"
 ...
 //PLOT MINBIAS AND FIT
   ...
   if(south_arm)
     corrbg_int->SetTitle("Yue Hang's Corr BG pT Integrated, South");  
   else
     corrbg_int->SetTitle("Yue Hang's Corr BG pT Integrated, North");  
   ...
   for(int arm = 0; arm < 2; arm++)
     {
       for(int bin = 0; bin < 100; bin++)
	 {
	   num_ratio[arm][bin] = Run15pAu_minbias[arm][bin];
	   cout << Run15pAu_minbias[arm][26] << endl;
	 }
     }

   // printed out values for both distributions at bin 26 (right around peak)
   double scale_peak[2] = {1824.94/(152.251),1945.03/(159.877)};

   // read in Yue hang pT integrated from sum of pt slices
   for(int bin = 0; bin < 100; bin++)
     {
       denom[0][bin] = (direct_pt_int_200mev_S[bin] + direct_pt_int_300mev_S[bin] + direct_pt_int_1gev_S[bin])/scale_peak[0];
       denom[1][bin] = (direct_pt_int_200mev_N[bin] + direct_pt_int_300mev_N[bin] + direct_pt_int_1gev_N[bin])/scale_peak[1];
       cout << denom[0][26] << endl;
       cout << denom[1][26] << endl;
     }
   ...
   for(int arm = 0; arm < 2; arm++)
       ...
       for(int bin = 0; bin < 100; bin++)
	 {
	   if(denom[arm][bin] != 0)
	     {
	       Run15pAu_ratio[arm][bin] = num_ratio[arm][bin]/denom[arm][bin];
	       ratio_errors[arm][bin] = 0.02*Run15pAu_ratio[arm][bin];
	     }
	 }
     }
   ...
   TGraphErrors *corrbg_int_ratio = new TGraphErrors(bins,x_array,Run15pAu_ratio[arm_low],x_errors[arm_low],ratio_errors[arm_low]);
